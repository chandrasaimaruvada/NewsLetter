...
public class NotificationServiceImpl implements NotificationService{
    ...
    @Override
    public String sendMailWithHTMLBody(EmailDataObj emailDataObj) {
        try {
            MimeMessage mimeMessage = javaMailSender.createMimeMessage();
            MimeMessageHelper mimeMessageHelper = new MimeMessageHelper(mimeMessage, true);

            mimeMessageHelper.setFrom(sender);
            mimeMessageHelper.setTo(emailDataObj.getReceipient());
            mimeMessageHelper.setSubject(emailDataObj.getMailSubject());
            mimeMessageHelper.setText(emailDataObj.getMailBody(), true);  // true indicates HTML content

            javaMailSender.send(mimeMessage);
            return "Successfully sent with HTML Body";
        } catch (Exception e) {
            return "Error in sendMailWithHTMLBody: " + e.getMessage();
        }
    }
